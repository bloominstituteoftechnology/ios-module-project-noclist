...
func getCompromisedAgentCount(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) -> Int {
    var compromisedAgentCount = 0
    for agent in agents {
...
let compromisedAgentAmount = getCompromisedAgentCount(agents: agents)
print("\(compromisedAgentAmount) agents have been compromised!")
...
let cleanAgentCount = findCleanAgents(agents: agents).count
print("\(cleanAgentCount) clean agents out of \(agents.count) total agents")
...
func findHighRisk(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
    for agent in agents {
        if agent.accessLevel >= 8 {
            var riskString = "\(agent.realName), level: \(agent.accessLevel)"
            if agent.compromised {
                riskString.append(" **WARNING** **COMPROMISED**")
            }
            print(riskString)
        }
    }
...
func agentGradeCount(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
    ...
    for agent in agents {
        ...
        case 8...:
            highLevelAgents += 1
        default:
            print("\(agent.realName) does not have an access level.")
        }
    }
...
printAgentsByAccess(agents: agents)
