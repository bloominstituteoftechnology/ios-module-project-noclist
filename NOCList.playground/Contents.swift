...
let nocAgents = [ethanHunt, jimPhelps, clairePhelps, eugeneKittridge, franzKrieger, lutherStickell, sarahDavies, maxRotgrab, hannahWilliams, jackHarmon, frankBarnes]


//: ## Step 3
//: Create a function that calculates the total number of compromised agents. Inside the function, iterate over the array of agents to determine which ones are compromised. Return the total count.
func calculateCompromisedAgents() -> Int {
    var total = 0
...
var compromisedAgents = calculateCompromisedAgents()

print("\(compromisedAgents) have been compromised!")
...
print("\(cleanAgents.count) clean agents out of \(nocAgents.count) total agents." )
...
func agentsByLevel() {
    ...
    print("\(lowLevel) low level agents, \(medLevel) mid level agents, and \(highLevel) high level agents")
}
...
func agentsByAccessLevel() {
    
    
    var nocAgents2 = nocAgents
    var sortedAgents = nocAgents2.sorted(by: { $0.accessLevel < $1.accessLevel} )
    
    for agent in sortedAgents {
        print("\(agent.coverName)'s access level is \(agent.accessLevel)")
    }
