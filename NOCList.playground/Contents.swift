...
let agentsArray: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]
    = [ethanHunt, jimPhelps, clairePhelps, eugeneKittridge, franzKrieger, lutherStickell, sarahDavies,
       maxRotGrab, hannahWilliams, jackHarmon, frankBarnes]

//: ## Step 3
//: Create a function that calculates the total number of compromised agents. Inside the function, iterate over the array of agents to determine which ones are compromised. Return the total count.
func numCompromised() -> Int {
    var count = 0
...
func findCleanAgents() -> [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] {
    var cleanAgentsArray: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] = []
    
    for agent in agentsArray {
        if !(agent.compromised) {
            cleanAgentsArray.append(agent)
...
print("\(cleanAgentsArray.count) clean agents out of \(agentsArray.count) total agents")
...
func findHighRisk() {
    for agent in agentsArray {
        if (agent.accessLevel >= 8) {
            print(agent.compromised ? "\(agent.realName), level: \(agent.accessLevel) **WARNING** **COMPROMISED**" : "\(agent.realName), level: \(agent.accessLevel)")
        }
    }
}
...
func printSortedAgentsArray() {
    let sortedAgentsArray = agentsArray.sorted(by: { $0.accessLevel < $1.accessLevel })
    for agent in sortedAgentsArray {
        print("\(agent.coverName), level: \(agent.accessLevel)")
    }
}

printSortedAgentsArray()
