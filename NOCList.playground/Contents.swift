...
func findCleanAgents() -> [String] {
    var arrayOfUncompromised: [String] = []
    
    for agent in agentArray {
        if !agent.compromised {
            print(agent.coverName)
            arrayOfUncompromised.append(agent.coverName)
        }
    }
    return arrayOfUncompromised
}
...
func findHighRisk()  {
    ...
    for agent in agentArray {
        if agent.accessLevel >= 8 {
            ...
            print("\(agent.realName), level: \(agent.accessLevel) \(warning)")
        }
    }
}
...
func sortAgents() {
    let x = agentArray.sorted {a,b in
