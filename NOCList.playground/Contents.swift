...
func findCleanAgents() -> [Any]{
    var isNotCompromised = [Any]()
    
    for agent in agents {
        if agent.compromised == false {
            isNotCompromised.append(agent)
        }
    }
...
func findHighRisk() {
    for agent in agents {
        if agent.compromised == true && agent.accessLevel >= 8 {
        print("\(agent.realName), \(agent.accessLevel) **WARNING** **COMPROMISED**")
        } else if agent.compromised == false && agent.accessLevel >= 8{
...
func agentLevels() {
    ...
    print("There are \(lowLevel) low level agents.")
           print("There are \(midLevel) mid level agents.")
           print("There are \(highLevel) high level agents.")
}
//: ## Step 10
