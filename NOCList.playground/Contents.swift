...
func findCleanAgents() -> [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]{
    var uncompromisedAgentList = [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]()
    for agent in agentList {
        if agent.compromised == false {
            print(agent.coverName)
            uncompromisedAgentList.append(agent)
        }
    }
    return uncompromisedAgentList
}
...
func totals() {
    var lowLevel: Int = 0 // <= 4
    var midLevel: Int = 0// 5...7
    var highLevel: Int = 0// >= 8
...
func printList(){
    let list = agentList.sorted(by: {$0.accessLevel < $1.accessLevel})
    for agent in list {
