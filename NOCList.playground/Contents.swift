...
let agent000 = (coverName: "Ethan Hunt", realName: "Tom Cruise", accessLevel: 8, compromised: false)
let agent001 = (coverName: "Jim Phelps", realName: "Jon Voight", accessLevel: 9, compromised: true)
let agent002 = (coverName: "Claire Phelps", realName: "Emmanuelle Beart", accessLevel: 5, compromised: false)
let agent003 = (coverName: "Eugene Kittridge", realName: "Henry Czerny", accessLevel: 10, compromised: true)
let agent004 = (coverName: "Franz Krieger", realName: "Jean Reno", accessLevel: 4, compromised: false)
let agent005 = (coverName: "Luther Stickell", realName: "Ving Rhames", accessLevel: 4, compromised: false)
let agent006 = (coverName: "Sarah Davies", realName: "Kristin Scott Thomas", accessLevel: 5, compromised: true)
let agent007 = (coverName: "Max RotGrab", realName: "Vanessa Redgrave", accessLevel: 4, compromised: false)
let agent008 = (coverName: "Hannah Williams", realName: "Ingeborga Dapkūnaitė", accessLevel: 5, compromised: true)
let agent009 = (coverName: "Jack Harmon", realName: "Emilio Estevez", accessLevel: 6, compromised: true)
let agent010 = (coverName: "Frank Barnes", realName: "Dale Dye", accessLevel: 9, compromised: false)
//: ## Step 2
...
func numberOfCompromisedAgents() -> Int { agents.filter({$0.compromised}).count }
//: ## Step 4
...
func findCleanAgents() -> [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] {
    let uncompromisedAgents = agents.filter({!$0.compromised})
...
func findHighRisk() {
    for agent in agents where agent.accessLevel >= 8 {
        let rnal = "\(agent.realName), \(agent.accessLevel)"
        let warning = agent.compromised ? "**WARNING** **COMPROMISED**" : ""
        print(rnal, warning, separator: " ")
    }
}
//: ## Step 8
...
func printCategorizedAgentCounts() {
    var count = (low: 0, mid: 0, high: 0)
    for agent in agents {
        switch agent.accessLevel {
        case 8...: count.high += 1
        case 5...7: count.mid += 1
        case ...4: count.low += 1
        default: fatalError()
        }
    }
    print("\(count.low) low level agents, \(count.mid) mid level agents, and \(count.high) high level agents")
}
//: ## Step 10
//: Call the above function and check its output in the console.
printCategorizedAgentCounts()
//: ## Step 11 (Optional)
...
func printCoverNamesAndAccessLevels() {
    agents.sorted(by: {$0.accessLevel < $1.accessLevel} )
        .forEach({ print("Cover name: \($0.coverName), Access level: \($0.accessLevel)") })
}

printCoverNamesAndAccessLevels()
