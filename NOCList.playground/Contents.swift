...
func determineNumOfCompromisedAgents(_ agents: [Agent]) -> Int {
    return agents.reduce(0, { num, agent in
        agent.compromised ? num + 1 : num
    })
}
//: ## Step 4
//: Call the above function to find the total number of compromised agents and then print a sentence that says "# agents have been compromised!" using string interpolation.
let numOfCompromisedAgents = determineNumOfCompromisedAgents(agents)
print("\(numOfCompromisedAgents) agents have been compromised!")
//: ## Step 5
...
func findHighRisk(_ agents: [Agent]) {
    for agent in agents {
        if agent.accessLevel >= 8 {
            print("\(agent.realName), level: \(agent.accessLevel) \(agent.compromised ? "**WARNING** **COMPROMISED**" : "")")
        }
    }
}
//: ## Step 8
...
func printAccessLevelTotals(_ agents: [Agent]) {
    var totals = (low: 0, mid: 0, high: 0)
    for agent in agents {
        ...
        case 8...:
            totals.high += 1
        default:
            print("Something is wrong with agent \(agent.coverName)")
        }
    }
    print("\(totals.low) low level agents, \(totals.mid) mid level agents, \(totals.high) high level agents")
}
//: ## Step 10
//: Call the above function and check its output in the console.
printAccessLevelTotals(agents)
//: ## Step 11 (Optional)
...
func printByAccessLevel(_ agents: [Agent]) {
    agents.sorted { a1, a2 in a1.accessLevel < a2.accessLevel }
        .forEach { agent in print("\(agent.coverName), level: \(agent.accessLevel)") }
}

printByAccessLevel(agents)
