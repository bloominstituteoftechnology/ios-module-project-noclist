...
let agent1 = (coverName: "Ethan Hunt", realName: "Tom Cruise", accesLevel: 8, compromised: false)
let agent2 = (coverName: "Jim Phelps", realName: "Jon Voight", accesLevel: 9, compromised: true)
let agent3 = (coverName: "Claire Phelps", realName: "Emmanuelle Beart", accesLevel: 5, compromised: false)
let agent4 = (coverName: "Eugene Kittridge", realName: "Henry Czerny", accesLevel: 10, compromised: true)
let agent5 = (coverName: "Franz Krieger", realName: "Jean Reno", accesLevel: 4, compromised: false)
let agent6 = (coverName: "Luther Stickell", realName: "Ving Rhames", accesLevel: 4, compromised: false)
let agent7 = (coverName: "Sarah Davies", realName: "Kristin Scott Thomas", accesLevel: 5, compromised: true)
let agent8 = (coverName: "Max RotGrab", realName: "Vanessa Redgrave", accesLevel: 4, compromised: false)
let agent9 = (coverName: "Hannah Williams", realName: "Ingeborga Dapkūnaitė", accesLevel: 5, compromised: true)
let agent10 = (coverName: "Jack Harmon", realName: "Emilio Estevez", accesLevel: 6, compromised: true)
let agent11 = (coverName: "Frank Barnes", realName: "Dale Dye", accesLevel: 9, compromised: false)
...
var secretAgents = [agent1, agent2, agent3, agent4, agent5, agent6, agent7, agent8, agent9, agent10, agent11]
...
func numberOfCompromisedAgents() -> Int {
    var compromisedNumber = 0
    for x in secretAgents {
        if x.compromised == true {
            compromisedNumber += 1
        }
    }
...
let number = numberOfCompromisedAgents()
print("\(number) agents have been compromised!")
...
func findCleanAgents() -> [String] {
    ...
    for x in secretAgents {
        if x.compromised == true {
            uncompromised.append(x.coverName)
        }
    }
    print(uncompromised)
...
let numberOfCleanAgents = cleanAgents.count
print("\(numberOfCleanAgents) clean agents out of \(secretAgents.count) total agents")
//: ## Step 7
...
func findHighRisk() {
    for x in secretAgents {
        if x.accesLevel >= 8 && x.compromised == true {
            print("\(x.realName), Level: \(x.accesLevel) **WARNING** **COMPROMISED**")
        }
        if x.accesLevel >= 8 && x.compromised == false {
            print("\(x.realName), Level: \(x.accesLevel)")
        }
    }
}
//: ## Step 8
