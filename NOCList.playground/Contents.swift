...
func findCleanAgents() -> Int {
    ...
    for agent in agents {
        if agent.compromised != true {
            cleanAgents += 1
            print("The cleaned agents are \(agent.coverName)")
        }
    }
    return cleanAgents
}
...
findCleanAgents()
print("\(findCleanAgents()) clean agents out of \(agents.count) total agents.")
...
func findHighRisk() {
    for agent in agents {
        ...
        } else if agent.accessLevel >= 8 && agent.compromised != false {
...
func lowMidHighLevelAgents() {
    ...
    for agent in agents {
        ...
        case 8...:
            highLevelAgent += 1
        default:
            print("There are no agents with in these levels")
        }
    }
    print("\(lowLevelAgent) low level agents, \(midLevelAgent) mid level agents, and \(highLevelAgent) high level agents")
}
//: ## Step 10
