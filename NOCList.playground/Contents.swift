...
let nocList = [ethanHunt, jimPhelps, clairePhelps, eugeneKittridge, franzKrieger, lutherStickell, sarahDavies, maxRotGrab, hannahWilliams, jackHarmon, frankBarnes]
...
func determineTotalCount() -> Int {
    var totalCount = 0
    for compromisedAgent in nocList{
...
func findCleanAgents() -> [Any] {
    var cleanAgents = [Any]()
    for uncompromisedAgents in nocList {
...
print("\(cleanAgentsArray.count) clean agents out of \(nocList.count)")
...
func findHighRisk() {
    _ = [Any]()
    for agentsAtHighRisk in nocList {
