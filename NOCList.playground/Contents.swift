...
func compromisedAgentCounter () -> Int {
    var count = 0
...
let compromisedAgents = compromisedAgentCounter()
...
func findCleanAgents () -> [Any] {
    var uncompromisedAgentArray: [Any] = []
    for agent in agentArray {
        if agent.compromised == false {
            print(agent.coverName)
            uncompromisedAgentArray.append(agent)
        }
    }
    return uncompromisedAgentArray
}
//: ## Step 6
...
let cleanAgentArray = findCleanAgents()
print("\(cleanAgentArray.count) clean agents out of \(agentArray.count) total agents.")
//: ## Step 7
...
func findHighRisk () {
    for agent in agentArray {
        if agent.accessLevel >= 8 && agent.compromised == false {
            print("\(agent.realName), level: \(agent.accessLevel)")
        } else if agent.accessLevel >= 8 && agent.compromised == true {
...
func levelGroupCounter () {
    ...
    for agent in agentArray {
        ...
        case 8...10:
            highCount += 1
        default:
            print("Error in levelGroupCounter function")
        }
    }
...
func nameAndLevelAscending () {
    let sortedArray = agentArray.sorted {
        $0.accessLevel < $1.accessLevel
    }
    for agent in sortedArray {
        print("\(agent.coverName), level: \(agent.accessLevel)")
    }
}

nameAndLevelAscending()
