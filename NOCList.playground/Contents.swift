...
func compromisedCalc() -> Int {
    
var compromisedCounter = 0
    
 for agents in agentArray {
    
    if (agents.compromised == true){
        
    compromisedCounter += 1
        
        }
    
    }
    return compromisedCounter
}

//: ## Step 4
//: Call the above function to find the total number of compromised agents and then print a sentence that says "# agents have been compromised!" using string interpolation.
print("\(compromisedCalc()) agents have been compromised!")
...
func findCleanAgents() -> [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] {
    ...
    var cleanAgents = [(coverName: "", realName: "", accessLevel: 0, compromised: false)]
    ...
    for agents in agentArray {
        ...
        if (agents.compromised == false){
            ...
            cleanAgents.append(agents)
        }
    
    }
    cleanAgents.remove(at: 0)
...
func agentLevel() {
    ...
    print("\(lowLevelAgents) of low level agents \(midLevelAgents) of mid level agents \(highLevelAgents) of high level agents")
}
...
func sortByAccessLevel() {
    var sortedArray = agentArray
    sortedArray.sort(by: {$0.accessLevel < $1.accessLevel})
    
    for agents in sortedArray{
        print("\(agents.coverName), Access Level: \(agents.accessLevel)")
}
}

print(sortByAccessLevel())
