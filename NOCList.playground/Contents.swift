...
let easternEuropeTeam = [ethanHunt, jimPhelps, clairePhelps, eugeneKittridge, franzKrieger, lutherStickell, sarahDavies, maxRotGrab, hannahWilliams, jackHarmon, frankBarnes]
...
func determineTotalCount() -> Int {
    var totalCount = 0
    for compromisedAgent in easternEuropeTeam {
        if compromisedAgent.compromised == true {
            totalCount += 1
        }
...
print("\(determineTotalCount()) agents have been compromised!")
...
func findCleanAgents() -> [Any] {
    var cleanAgents = [Any]()
    for uncompromisedAgents in easternEuropeTeam {
        if uncompromisedAgents.compromised == false {
...
let cleanAgentsArray = findCleanAgents()
print("\(cleanAgentsArray.count) clean agents out of \(easternEuropeTeam.count)")
...
func findHighRisk() {
    var highRiskAgents = [Any]()
    for agentsAtHighRisk in easternEuropeTeam {
        if agentsAtHighRisk.accessLevel > 8 && agentsAtHighRisk.compromised == true {
            print("\(agentsAtHighRisk.realName) **WARNING** **COMPROMISED**")
        } else if agentsAtHighRisk.accessLevel > 8 && agentsAtHighRisk.compromised == false {
            print("\(agentsAtHighRisk.realName)")
        }
    }
}
...
func totalOfLevels() {
    var highAccessAgents = 0
    var midAccessAgents = 0
    var lowAccessAgents = 0
    for agent in easternEuropeTeam {
    ...
    print("\(highAccessAgents) high level agents, \(midAccessAgents) mid level agents, \(lowAccessAgents) low level agents")
}
...
func findBurntAgents() -> [Any] {
    var burntAgents = [Any]()
    for uncompromisedAgents in easternEuropeTeam {
        if uncompromisedAgents.compromised == false {
            print(uncompromisedAgents.coverName)
            burntAgents.append(uncompromisedAgents)
        }
    }
    return burntAgents
}
var burntAgentsArray = findBurntAgents()
var agentsAndAccessLevel = cleanAgentsArray + burntAgentsArray
