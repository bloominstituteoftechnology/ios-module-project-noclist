...
let allAgents = [ethanHunt, jimPhelps, clairePhelps, eugeneKittridge, frankBarnes, lutherStickell, sarahDavies, maxRotGrab, hannahWilliams, jackHarmon, frankBarnes]
...
func compromisedAgents() -> Int {
    ...
    for compromisedAgents in allAgents {
        if compromisedAgents.compromised == true {
            totalCompromised += 1
        }
    }
...
func findCleanAgents() -> Int {
    var totalUncompromised = 0
    
    for uncompromisedAgents in allAgents {
        if uncompromisedAgents.compromised == false {
            totalUncompromised += 1
            print("The uncompromised agents are \(uncompromisedAgents.coverName)")
        }
    }
    
    return totalUncompromised
}
...
print("\(findCleanAgents()) clean agents out of \(allAgents.count)")
...
func findAgentLevels() {
    ...
    for findAgentLevel in allAgents {
        let agentList = findAgentLevel.accessLevel
        
    switch agentList {
    case 1...4:
        print("\(agentList) low level agents")
    case 5...7:
        print("\(agentList) mid level agents")
    case 8...100:
        print("\(agentList) high level agents")
    default:
        print("This is not on the level of agents")
            break
    }
}
...
func ascendingOrder() {
    ...
    let sortedList = allAgents.sorted(by: { $0.accessLevel < $1.accessLevel})
